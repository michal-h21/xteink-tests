...
\linebreakersetup{maxemergencystretch=0.6em}
...
\begin{lstlisting}
$ python nosplit.py --downscale bicubic --clean --2bit --contrast-boost 2 --no-dither  --no-split sample.pdf 
\end{lstlisting}


\subsection{Další pokus s Epubem}

Ty XTCH soubory jsou hodně velký, každá stránka má konstantní velikost protože nefunguje žádná komprese. \texttt{cbz2xtc.py} ale vytváří
dočasný PNG soubory, který můžeme zachovat, když nepoužijeme volbu \texttt{--clean}. Nacházejí se v adresáři \texttt{.temp\_png/}, kde 
se pro každej PDF soubor vytváří podadresář. V našem případě můžeme vytvořit Epub soubor pomocí \texttt{img2epub.lua}:

\begin{lstlisting}
$ texlua img2epub.lua .temp_png/sample/ sample.epub
\end{lstlisting}

Výslednej soubor má velikot zhruba čtvriny XTCH souboru, ale Crosspoint to
vykreslí úplně tragicky. Jednak obrázek trošku zmenší a navíc očividně udělá
dithering a černobílej obrázek. Výsledek je dost nečitelnej.
...
\begin{enumerate}
  \item Zkusit vytvořit Epub s 16 barvama, ale bez ditheringu, a zkusit ho otevřít v Papyrusu. Třeba to bude vypadat líp než v Crosspointu.
  \item Hlavně chci vyzkoušet dostatečně dlouhej řádek v enumerate\footnote{A jak vlastně bude vypadat poznámka pod čarou? Klidně může bejt trošku delší}
\end{enumerate}
...
\begin{itemize}
  \item Zkusit vytvořit Epub s 16 barvama, ale bez ditheringu, a zkusit ho otevřít v Papyrusu. Třeba to bude vypadat líp než v Crosspointu.
  \item Hlavně chci vyzkoušet dostatečně dlouhej řádek v enumerate
    \begin{enumerate}
      \item tohle je zanořenej list
      \item a další položka. Je důležitý, aby byla dostatečně dlouhá, aby se zalomila a otestovala zalamování řádků v enumerate.
      \item ve skutečnosti potřebuju další položku, protože tady dochází k zalomení stránky
    \end{enumerate}
\end{itemize}


A jak vlastně vypadájí tabulky?
...
Nějaký text po tabulce.

No a taky musíme vyzkoušet matematiku:

\[
  f(x) = \int_{-\infty}^\infty \hat{f}(\xi) e^{2\pi i \xi x} d\xi
\]

Tady pokračujeme dál, abychom otestovali zalamování řádků. A ještě zkusíme nějakou delší větu, která se nevejde do jednoho řádku a bude se muset zalomit.

No a co třeba verbatim?
\begin{verbatim}
$ echo "ahoj"
\end{verbatim}
Další text
